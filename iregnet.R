...
# -c(3:10, 26, 28)
folds.csv.vec <- folds.csv.vec[-c(3:10, 26, 28:33)]
pred.err.list <- list()
  ...
  for(test.fold in u.fold.vec){
    ...
    fun.list <- list(
      iregnet.scale1=function(inputs, outputs){
        iregnet::cv.iregnet(
          inputs, outputs,
          family="gaussian", estimate_scale=FALSE, scale_init=1, threshold = 1e-4)
      },
      iregnet.estscale=function(inputs, outputs){
        iregnet::cv.iregnet(
          inputs, outputs, family="gaussian", threshold = 1e-4)
      },
    ...
    error_status <- list(constant = 0, 
                         iregnet.scale1 = fit.list$iregnet.scale$error_status,
                         iregnet.estscale = fit.list$iregnet.estscale$error_status, 
                         penaltyLearning.scale1 = 0)
    pred.list <- list(
    ...
    for(algorithm in names(pred.list)){
      ...
      pred.err.list[[paste(data.name, test.fold, algorithm)]] <- cbind(pred.err.list[[paste(data.name, test.fold, algorithm)]], 
                                                                       data.table("errorid" = error_status[[paste(algorithm)]]))
    }
  }
}
(pred.err <- do.call(rbind, pred.err.list))
write.xlsx(pred.err, "~/development/gsoc/benchsheet.xlsx", asTable = TRUE)
